														...
														<meta charset="utf-8">
															...
															<title> Providus option Earning History    </title>
...
<div class="pusher push-trading">
 ...
 <section class="img-bg-section">
     <div class="row">
        ...
        <div class="top-info">
            <h2 class="title">Earning History</h2>
        </div>
     </div>
...
<div style="">
												<section class="content-box history-min">
													<div class="row">
														<div class="table" style="overflow-y:auto;">
																							<?php 
                                                    $query = mysqli_query($conn, "SELECT * FROM earning_tb WHERE ea_uid = '$session_id'");
													...
													<table class="" style=" min-width:600px;">
														...
														<?php
									$query_st = mysqli_query($conn," SELECT * FROM earning_tb WHERE ea_uid = '$session_id'");
									$counter = 0;
									while(@$row = mysqli_fetch_array($query_st))
									{
										
										$id = $row['ea_id'];
										$result_date = $row['ea_date'];
										$state = $row['ea_status'];
										$counter++;
										if($row['ea_status'] =='Pending' )
											{
											$deposit_status = "Pending";
											
											}else
											if($row['ea_status'] =='Earning Pending' )
											{
											$deposit_status = "<font color='#FF6600'> Earning Pending</font>";
											
											}else
											if($row['ea_status'] =='Processing...' )
											{
											$deposit_status = "<font color='#FF6600'> Processing...</font>";
											
											}else
											$deposit_status = "<font color='#00CC66'> Successful</font>";
											@$acct_amt2 = number_format($row['ea_amt']);
											if($acct_amt2 == '')
											{
												$acct_amt2 = '0';
											}else
		...
		echo $message = '<h3 align="center"><div class="alert alert-dismissible alert-primary">
		  ...
		  <strong>Sorry!</strong> You currently do not have any earning history record at the moment. 
